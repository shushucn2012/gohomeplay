...
public class CalendarPickerView extends ListView {
    ...
    public CalendarPickerView(Context context, AttributeSet attrs) {
        ...
        titleTextColor =
                a.getColor(R.styleable.CalendarPickerView_titleTextColor, getResources().getColor(R.color.calendar_text_active));
        headerTextColor =
                a.getColor(R.styleable.CalendarPickerView_headerTextColor, getResources().getColor(R.color.calendar_text_active));
        a.recycle();
    ...
    private boolean doSelectDate(Date date, MonthCellDescriptor cell) {
        ...
        if (date != null) {
            ...
            if (selectionMode == SelectionMode.RANGE && selectedCells.size() > 1) {
                ...
                selectedCells.get(0).setStart(true);
                selectedCells.get(1).setEnd(true);
                    ...
                    for (List<MonthCellDescriptor> week : month) {
                        for (MonthCellDescriptor singleCell : week) {
                            if (singleCell.getDate().after(start)
                                ...
                                singleCell.setStart(false);
                                singleCell.setEnd(false);
