 ...
 * 行事历首页日历提示图
 */
public class CalandarMainTipPopWin extends PopupWindow {

    private Context mContext;
    private int touchCount = 0;//点击次数

    public CalandarMainTipPopWin(Context context) {
        super(context);
        ...
        toolView.setOnTouchListener(new OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    //dismiss();
                    if (touchCount == 0) {
                        root_view.setBackgroundResource(R.mipmap.calendar_guide_2);
                    } else if (touchCount == 1) {
                        root_view.setBackgroundResource(R.mipmap.calendar_guide_3);
                    } else if (touchCount == 2) {
                        root_view.setBackgroundResource(R.mipmap.calendar_guide_4);
                    } else {
                        dismiss();
                    }
                    touchCount++;
