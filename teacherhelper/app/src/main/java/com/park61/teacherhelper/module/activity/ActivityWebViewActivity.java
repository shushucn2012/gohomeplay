...
public class ActivityWebViewActivity extends BaseActivity {
    ...
    public void initData() {
        ...
        String url = "http://m.61park.cn/t/html-sui/activity/activity-index.html?activityId=" + activityId;
        url_share = url;
        logout("url======" + url);
        syncCookie(url);
        ...
        wv.loadUrl(url);
        mShareInfoBean = new ShareInfoBean(url, getIntent().getStringExtra("picUrl"), "培训活动", "更多精彩，尽在61学院！");
        asyncGetActivityDetail();
    }

    private void asyncGetActivityDetail() {
        String wholeUrl = AppUrl.host + AppUrl.activityDetail;
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("activityId", activityId);//activityId=1
    ...
    BaseRequestListener blistener = new JsonRequestListener() {
        ...
        public void onSuccess(int requestId, String url0, JSONObject jsonResult) {
            ...
            int paymentStatus = jsonResult.optInt("paymentStatus");
            if (paymentStatus == 0) {
                area_no_pay.setVisibility(View.VISIBLE);
                btn_comein.setVisibility(View.GONE);
            } else if (paymentStatus == 1) {
                area_no_pay.setVisibility(View.GONE);
                btn_comein.setVisibility(View.VISIBLE);
            }
            tv_price.setText("打赏入场：￥" + jsonResult.optString("price"));
            mShareInfoBean = new ShareInfoBean(url_share,
                    mActivityDetailsBean.getCoverImg(),
                    mActivityDetailsBean.getName(),
                    mActivityDetailsBean.getIntroduction());
        }
    };

    @Override
    public void initListener() {
        mBCActOrderRefresh = new BCActOrderRefresh(new BCActOrderRefresh.OnReceiveDoneLsner() {
            @Override
            public void onGot(Intent intent) {
                asyncGetActivityDetail();
            }
        });
        mBCActOrderRefresh.registerReceiver(mContext);
        ...
        area_to_pay.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                GlobalParam.ORDER_TYPE = "teacher_meet";
        ...
        btn_comein.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent it = new Intent(mContext, FreeContentWebViewActivity.class);
                it.putExtra("PAGE_TITLE", mActivityDetailsBean.getName());
                it.putExtra("picUrl", mActivityDetailsBean.getCoverImg());
                it.putExtra("intro", mActivityDetailsBean.getIntroduction());
                it.putExtra("activityId", activityId);
                it.putExtra("payst", 1);
                startActivity(it);
            }
        });
        area_free_in.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent it = new Intent(mContext, FreeContentWebViewActivity.class);
                it.putExtra("PAGE_TITLE", mActivityDetailsBean.getName());
    ...
    public void onDestroy() {
        super.onDestroy();
        mBCActOrderRefresh.unregisterReceiver(mContext);
    }
